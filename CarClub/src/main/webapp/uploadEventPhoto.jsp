...
<head>
  ...
  <title>Event Photos - Car Club</title>
...
<%
  ...
  // Only club managers need this page — load events they manage
    ...
    if ("delete".equals(action)) {
      ...
      if (photoIDStr != null && !photoIDStr.trim().isEmpty()) {
        try {
          ...
          boolean ok = MysqlCon.deleteEventPhoto(photoID, sessionUserID);
          if (ok) successMsg = "Photo removed successfully.";
          else    errorMsg   = "Could not remove photo.";
    ...
    // ── Add photo ─────────────────────────────────────────────────────────
      ...
      if (eventIDStr == null || eventIDStr.trim().isEmpty()) {
        ...
        try {
          ...
          boolean ok = MysqlCon.addEventPhoto(eventID, sessionUserID, photoURL.trim());
          if (ok) {
            successMsg = "Photo added successfully!";
          } else {
            errorMsg = "Failed to add photo. Make sure you manage this event's club.";
          }
        } catch (NumberFormatException e) {
          errorMsg = "Invalid event selection.";
        }
      }
    }
  }

  // ── Selected event for gallery preview ────────────────────────────────────
  ...
  if (selectedEventIDStr != null) {
    try {
      ...
      currentPhotos = MysqlCon.getEventPhotos(selectedEventID);
...
<div class="page-layout">
  ...
  <div class="left-panel">
    ...
    <h1 class="left-title">Event<br><span>Photos</span></h1>
    <p class="left-sub">
      Capture the moment. Add photo URLs to events you manage so attendees can relive the experience.
    </p>
    <div class="perks">
      <div class="perk">
      ...
      <div class="perk">
        <div class="perk-dot"></div>
        <div class="perk-text"><strong>Public Gallery</strong> – photos display on the event listing page.</div>
      </div>
    </div>
  </div>
  ...
  <div class="right-panel">
    ...
    <% if (managedEvents.isEmpty()) { %>
      <div class="no-events-notice">
        ...
        <a href="createEvent.jsp">Create an event first</a> to upload photos.
      </div>
    <% } else { %>

      <div class="form-heading">Add a photo</div>

      <form method="POST" action="uploadEventPhoto.jsp" novalidate>
        ...
        <div class="form-group">
          ...
          <input type="url" id="photoURL" name="photoURL"
                 placeholder="https://example.com/event-photo.jpg" maxlength="255" required/>
      ...
      <!-- ── Current photos for selected event ───────────────────────────── -->
      <% if (selectedEventID > 0) { %>
        <div class="photo-section-label">
          Photos for "<%= selectedEventName %>" (<%= currentPhotos.size() %>)
        </div>

        <% if (currentPhotos.isEmpty()) { %>
          <p class="no-photos">No photos yet for this event. Add one above.</p>
        <% } else { %>
          <div class="photo-grid">
            <% for (String[] photo : currentPhotos) { %>
              <div class="photo-card">
                <img src="<%= photo[2] %>" alt="Event photo"
                ...
                <div class="photo-card-footer">
                  ...
                  <form method="POST" action="uploadEventPhoto.jsp" style="display:inline;"
